...
####################################################################################################################################
ds = xr.open_dataset("../process/Fig04_E3SM_CLIM_STA_clim_P1.nc")
clim_STA_P1 = ds['clim']

ds = xr.open_dataset("../process/Fig04_E3SM_KE_TP_forcing_P1.nc")
SST_comp_TP_P1 = ds['SST_comp']; SST_pval_TP_P1 = ds['SST_p_value']
ds = xr.open_dataset("../process/Fig04_E3SM_KE_TP_comp_P1.nc")
STA_comp_TP_P1 = ds['STA_comp']; STA_pval_TP_P1 = ds['STA_p_value']
SLP_comp_TP_P1 = ds['SLP_comp']; SLP_pval_TP_P1 = ds['SLP_p_value']
UWD_comp_TP_P1 = ds['UWD_comp']; UWD_pval_TP_P1 = ds['UWD_p_value']
VWD_comp_TP_P1 = ds['VWD_comp']; VWD_pval_TP_P1 = ds['VWD_p_value']

ds = xr.open_dataset("../process/Fig04_E3SM_KE_NP_forcing_P1.nc")
SST_comp_NP_P1 = ds['SST_comp']; SST_pval_NP_P1 = ds['SST_p_value']
ds = xr.open_dataset("../process/Fig04_E3SM_KE_NP_comp_P1.nc")
STA_comp_NP_P1 = ds['STA_comp']; STA_pval_NP_P1 = ds['STA_p_value']
SLP_comp_NP_P1 = ds['SLP_comp']; SLP_pval_NP_P1 = ds['SLP_p_value']
UWD_comp_NP_P1 = ds['UWD_comp']; UWD_pval_NP_P1 = ds['UWD_p_value']
VWD_comp_NP_P1 = ds['VWD_comp']; VWD_pval_NP_P1 = ds['VWD_p_value']

ds = xr.open_dataset("../process/Fig04_E3SM_KE_TPNP_forcing_P1.nc")
SST_comp_TPNP_P1 = ds['SST_comp']; SST_pval_TPNP_P1 = ds['SST_p_value']
ds = xr.open_dataset("../process/Fig04_E3SM_KE_TPNP_comp_P1.nc")
STA_comp_TPNP_P1 = ds['STA_comp']; STA_pval_TPNP_P1 = ds['STA_p_value']
SLP_comp_TPNP_P1 = ds['SLP_comp']; SLP_pval_TPNP_P1 = ds['SLP_p_value']
UWD_comp_TPNP_P1 = ds['UWD_comp']; UWD_pval_TPNP_P1 = ds['UWD_p_value']
VWD_comp_TPNP_P1 = ds['VWD_comp']; VWD_pval_TPNP_P1 = ds['VWD_p_value']
...
ax.set_title('Tropical Pacific minus CTRL\n\n', fontsize=14, x=0.5, weight='bold'); ax.set_title('          SST forcing from P1 (1977-1999)', loc='left', fontsize=13)
comp = SST_comp_TP_P1; pval = SST_pval_TP_P1
...
comp = SST_comp_NP_P1; pval = SST_pval_NP_P1
cf = comp_map(ax, comp.where(comp!=0), pval, domain, cmap, levs, transform_map)
ccrs_grid(ax, np.arange(20,360.1,80), np.arange(-30,60.1,30), 13)

ax = plt.subplot(gs[0,2], projection=projection_map); ax.text(-0.09, 1.1, "c", fontsize=20, weight='bold', transform=ax.transAxes)
ax.set_title('Tropical-North Pacific minus CTRL\n\n', fontsize=15, x=0.5, weight='bold'); ax.set_title('          SST forcing from P1 (1977-1999)', loc='left', fontsize=13)
comp = SST_comp_TPNP_P1; pval = SST_pval_TPNP_P1
cf = comp_map(ax, comp.where(comp!=0), pval, domain, cmap, levs, transform_map)
ccrs_grid(ax, np.arange(20,360.1,80), np.arange(-30,60.1,30), 13)
...
comp = STA_comp_TP_P1; pval = STA_pval_TP_P1; clim = data_domain(clim_STA_P1, domain)
...
comp = STA_comp_NP_P1; pval = STA_pval_NP_P1; clim = data_domain(clim_STA_P1, domain)
STL  = clim.lat[clim.argmax(axis=0)].sel(lon=slice(140,238)).rolling(lon=4, center=True).mean()
cf = comp_map(ax, comp.where(comp!=0)/2., pval, domain, cmap, levs, transform_map)
cs = ax.contour(clim.lon, clim.lat, clim, colors='grey', levels=[4,8,12], linewidths=1.3, transform=transform_map)
ax.clabel(cs, inline=True, fontsize=13, fmt='%d', colors='k')
...
comp = STA_comp_TPNP_P1; pval = STA_pval_TPNP_P1; clim = data_domain(clim_STA_P1, domain)
STL  = clim.lat[clim.argmax(axis=0)].sel(lon=slice(140,238)).rolling(lon=4, center=True).mean()
cf = comp_map(ax, comp.where(comp!=0)/2., pval, domain, cmap, levs, transform_map)
cs = ax.contour(clim.lon, clim.lat, clim, colors='grey', levels=[4,8,12], linewidths=1.3, transform=transform_map)
ax.clabel(cs, inline=True, fontsize=13, fmt='%d', colors='k')
...
comp  = SLP_comp_TP_P1;  pval = SLP_pval_TP_P1
compu = UWD_comp_TP_P1; pvalu = UWD_pval_TP_P1
compv = VWD_comp_TP_P1; pvalv = VWD_pval_TP_P1
cf = comp_map(ax, comp.where(comp!=0)/2., pval, domain, cmap, levs, transform_map)
...
comp  = SLP_comp_NP_P1;  pval = SLP_pval_NP_P1
compu = UWD_comp_NP_P1; pvalu = UWD_pval_NP_P1
compv = VWD_comp_NP_P1; pvalv = VWD_pval_NP_P1
cf = comp_map(ax, comp.where(comp!=0)/2., pval, domain, cmap, levs, transform_map)
qv = quiver_comp(ax, compu/2., pvalu, compv/2., pvalv, domain, transform_map, step=5, scale=13, siglev=0.1)
ccrs_grid(ax, np.arange(120,240.1,30), np.arange(20,60.1,10), 12); ccrs_plot(ax, 140, 175, 31, 45)
...
comp  = SLP_comp_TPNP_P1;  pval = SLP_pval_TPNP_P1
compu = UWD_comp_TPNP_P1; pvalu = UWD_pval_TPNP_P1
compv = VWD_comp_TPNP_P1; pvalv = VWD_pval_TPNP_P1
cf = comp_map(ax, comp.where(comp!=0)/2., pval, domain, cmap, levs, transform_map)
qv = quiver_comp(ax, compu/2., pvalu, compv/2., pvalv, domain, transform_map, step=5, scale=13, siglev=0.1)
ccrs_grid(ax, np.arange(120,240.1,30), np.arange(20,60.1,10), 12); ccrs_plot(ax, 140, 175, 31, 45)
